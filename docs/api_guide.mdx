...
In addition to the actual theme configuration parameters the `FlexSubThemesData` configuration has a few extra settings that are used to control the behavior of the component themes.

* [**useTextTheme**](https://pub.dev/documentation/flex_color_scheme/latest/flex_color_scheme/FlexSubThemesData/useTextTheme.html): (**true**/false). When opting in on using the component sub-themes, this flag controls if the text theme that uses Material 3 font sizes and letter spacing, is also used. For the time being, until the [Typography for Material 3 lands in Flutter stable](https://github.com/flutter/flutter/pull/97829), a custom [**m3extTheme**](https://pub.dev/documentation/flex_color_scheme/latest/flex_color_scheme/FlexColorScheme/m3TextTheme-constant.html) "internal" replica with its geometry is used when this is enabled, which it is by default. **IMPORTANT** this temporary M3 TextTheme only supports "English like" text geometry, or Typography. If you are using a locale that does not use "English like" character types, then you may have to set this flag to **false** to opt-out of using this text theme and geometry for now, in order for letter spacing to work as intended. This issue will be resolved when M3 Typography lands in Flutter stable channel, and FlexColorScheme can be changed to use it, instead of the custom temporary version it now uses as M3 TextTheme and geometry.
...
* [**bottomNavigationBar**](https://pub.dev/documentation/flex_color_scheme/latest/flex_color_scheme/FlexSubThemes/bottomNavigationBar.html) used to create the component theme for the [BottomNavigationBarWidget](https://api.flutter.dev/flutter/material/BottomNavigationBar-class.html).
...
You can also use these static component theme helpers without using FlexColorScheme to define custom components themes, or even without using FlexColorScheme at all. You can further modify the component theme helpers' produced results by using their `copyWith` method. The helpers may be simpler to use than the raw component theme constructors, or your can just study them to see how some component theme that interests you, is defined.
