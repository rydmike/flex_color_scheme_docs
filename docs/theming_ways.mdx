...
Flutter 2.10 stable some widgets still depend e.g. on `primaryColorLight`
and `primaryColorDark` as their default colors, for example `CircleAvatar`. If you don't give them
`ColorScheme` derived values, they will use the default swatch theme based
colors. What happens in the next new stable release of Flutter we will see.
...
important. If you do not, you will may get background color defaults in
...
of course requires that your `ColorScheme` uses correct background
...
versitale seed generated `ColorScheme`, and "a few" built-in color schemes
that can be nice starting points too. To top it all off, it can help you
