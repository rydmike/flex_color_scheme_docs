...
too which is useful in this case.
 ...
 the **Themes Playground** app can show it to you. Copy the "M3 Baseline" theme
 to the customizable theme and then select the primary color having color
 *0xff6750a4**. It will on the wheel picker draw its **Material Color** swatch
 created using this color using the same `createPrimarySwatch` function. Below
 it you will also see the primary tonal palette for the same input color.
...
Let's call this group of theme **Swatch** based, since they all either
directly or under the hood via defaults, use the old `MaterialColor`
color swatches to create `ThemeData` and its colors.
...
Since the methods do not accept a `ColorScheme` as input, we will use
`MaterialColor` swatch as input, and we will use our custom `flexSwatch`.
...
all of them with `copyWith`. Still if we create our `ThemeData` like this:
