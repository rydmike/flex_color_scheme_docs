...
and `primaryColorDark` as their default colors, for example `CircleAvatar`.
If you don't give them `ColorScheme` derived values, they will use the default
swatch theme based colors.
...
it is of course requires that your `ColorScheme` uses correct background
...
surface blending and a more versitale seed generated `ColorScheme`,
