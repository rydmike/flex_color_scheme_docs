...
the above three `FleXColorScheme` based results.
...
based color assignments on the colors `ThemeData`, `primaryColorLight` and
...
In dark theme mode as before the peculiar grey scales on `primaryColor`,
`primaryColorLight`, `primaryColorDark` and `secondaryHeaderColor` prevails,
as does the teal color on the `toggleableActiveColor` instead of
...
the Flutter SDK examples above, has the closest equivalent in method
6 earlier.

We can see that it has no legacy `ThemeData` direct
color properties that do not match and follow the color design in the
...
direct color properties that do not match and follow the color design in the
used `ColorScheme`, this applies to both light and dark theme mode.

This scheme also uses a Material 3 seed generated `ColorScheme` that is
identical to the one produced by Flutter's only directly supported `ColorScheme`
generation algorithm. FlexColorScheme offers many ways to tune the algorithm.
...
This is an opinionated default that comes when we enable and opt-in on
using `FlexSubThemesData` like we did in the setup for example.
It can also be turned OFF and reverting to `ThemeData` defaults when using
the sub-themes by setting by setting its `interactionEffects` to false.
...
How should you create your theme when using vanilla Flutter theming to get
a modern `ColorScheme` based theme?

As shown above, your simplest approach is to use the `ThemeData.from` factory,
with whatever `ColorScheme` you have defined.

The drawback with this methods is that not all colors in `ThemeData` will get
colors matching your `ColorScheme`. Additionally you cannot define any component
themes in same theme definition, nor adjust any of the direct color properties
in `ThemeData` that you may want to modify from their defaults where
`ThemeData.from` leaves gaps. You can of course apply all of these using
`copyWith`, and you should definitively do so.

The second option is to just use the big `ThemeData` factory. This gives you
access to all the properties in it and you can set and define them as you
please, as long as you are familiar with all its inner workings, this is fine
and not so difficult. I recommend starting with a setup the replicates what
the `ThemeData.from` factory does and adjusting it as desired.

```dart
// Assuming you have defined your `colorScheme`:
ThemeData(
  colorScheme: colorScheme,
  brightness: colorScheme.brightness,
  // Prefer this instead for the `ThemeData.primaryColor`
  primaryColor: colorScheme.primary,
  // These you have to compute based based on your colorScheme primary color,
  // if you want to give them scheme matching values, here is an example that
  ...
  // You can use other colors too, e.g. if you want some surface branding on
  // them then the above `alphaBlend` is the way to add it.
  scaffoldBackgroundColor: colorScheme.background,
  canvasColor: colorScheme.background,
  backgroundColor: colorScheme.background,
  cardColor: colorScheme.surface,
...
`flexSchemeLight` for or light theme mode and `flexSchemeDark`, and remove
the `myTextTheme` and component themes, we can try this custom `ThemeData`
setup in the same demo app we used for all the above cases.

Below we compare this to FlexColorScheme case 10, which it with respect to
`ColorScheme` and `ThemeData` colors is equal to.
...
been migrated to use default from `ThemeData.colorScheme`.

As the migration and deprecation of `ThemeData` colors in Flutter SDK progresses,
the importance of doing this will become less important. Some widgets still
depend on `primaryColorLight` and `primaryColorDark` as their default
colors, for example `CircleAvatar`.
...
If you do not, you will get those ancient very grey background colors we saw in
the swatch based themes in dark mode above.

This of course requires that your `ColorScheme` uses correct background
colors and onColors. Use the build in `ColorScheme.light` and `ColorScheme.dark`
for that, or even a `ColorScheme.fromSeed` as a base, and define you custom
colors in them as needed to make your custom light and dark `ColorScheme`.

If all the above felt a bit verbose or complicated, then **FlexColorScheme** will
do all the above color setups for you. Plus it has surface blending and more
versitale seed generated `ColorScheme`, and a few built-in
`ColorScheme`s that can be nice starting points too. To top it off, it can help
you with component theming too.
