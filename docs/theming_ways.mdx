...
5. ThemeData.from(colorScheme: ColorScheme.fromSwatch())
6. ThemeData(colorScheme: ColorScheme())
7. ThemeData.from(colorScheme: ColorScheme())
8. ThemeData(colorSchemeSeed: Color()),
9. ThemeData(colorScheme: ColorScheme.fromSeed(seedColor: Color())),
10. ThemeData.from(colorScheme: ColorScheme.fromSeed(seedColor: Color())),
11. FlexThemeData.light(colorScheme)
12. FlexThemeData.light(colorScheme, keyColors)
13. FlexThemeData.light(colorScheme, keyColors, subThemesData)
...
```dart
// 1) TD.light - ThemeData.light() / .dark()
title: '1) TD.light',
...
```dart
// 2) TD brightness - ThemeData(brightness: ...)
//
title: '2) TD brightness',
...
```dart
// 3 blue) TD primarySwatch - ThemeData(brightness: ...)
...
All using our `flexSwatch` as input will give us, is just a variation with a
different color, but let's do it too so we can compare it later to its
`ColorScheme` based versions.

```dart
// 3 M3 base) TD primarySwatch - ThemeData(brightness: ...)
title: '3) TD primarySwatch',
theme: ThemeData(
  brightness: Brightness.light,
...
We can also create a `ThemeDat` a from a `ColorScheme` that is created from
...
//
title: '4) TD colorScheme.fromSwatch',
theme: ThemeData.from(
  colorScheme: ColorScheme.fromSwatch(
    primarySwatch: flexSwatch, // or Colors.blue
    brightness: Brightness.light,
  ),
),
 ...
 * ColorScheme background in light theme mode color has the wrong contrasting
   onColor. This is very bad, and probably a bug, or at least an oversight.
   Why does it get this color?

We can track that from `ThemeData` and see that if we have not passed `colorScheme`
to it, it will be created with `colorScheme ??= ColorScheme.fromSwatch`, in it
we will find:
...
```dart
  ...
  factory ColorScheme.fromSwatch({
    ...
    return ColorScheme(
      ...
      // because the same primaryIsDark boolean is used. That check was for
      ...
      // so for most swatches we get the wrong on onColor contrasting color for
      // onBackground with all these theme creation methods. You would have to
      // know this and correct with a copyWith.
...
So what about the 4th option, we are using ...
