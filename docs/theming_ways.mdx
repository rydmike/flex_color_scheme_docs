...
colors assigned to legacy colors in `ThemeData`. We can in light mode
...
scheme, but the assignment is correct). We also see the error color got moved
over to THemeData. In fact this applies to canvas, scaffold, card and many
other colors too. We can take a peek at the `ThemeData.from` factory to get
the full scoop here:
...
still being used for legacy reasons. If we look closely we see that in
light theme mode, we get got `colorScheme.secondary` on the
...
by on/off switches, and in dark theme mode that is now still "teal" colored,
so they will get a teal color that has nothing with our `ColorScheme`, unless
we apply it to it separately. But why did it get the `secondary` in light
theme mode and not in dark theme mode, after all it is not even assigned any
value in `ThemeData.from`. That is because `ThemeData` itself does thin thing_
...
It is understandable that logic was kept for all the swatch based themes backwards
compatibility, but why could `ThemeData.from` not just have also
included a line that set._All

```dart
toggleableActiveColor: colorScheme.secondary,
```

Along with accent color, back when it was introduced it would have been fine,
and we would have had a slightly less confusing `ThemeData` from a `ColorScheme`
result.

Next we have to wonder about three color that `ThemeData.from` happily ignores
...
`secondaryHeaderColor` are still the same blue as when we used swatch colors.
That is because in ThemeData they get colors from the swatch, that has not
been assigned any value, so it has defaulted `Colors.blue`.
...
in Flutter, so this means they like the switches in dark mode now use colors
that has nothing to do with the `ColorScheme` we wanted our application to
have.
...
<table>
  <tr>
    ...
    <td>&nbsp;</td>
    <td><b>10) FTD.light(colorScheme)</b></td>
    <td>&nbsp;</td>
    <td><b>11) FTD.light(keyColors)</b></td>
    <td>&nbsp;</td>
...
**TODO: Review and explain differences - recommendations**
