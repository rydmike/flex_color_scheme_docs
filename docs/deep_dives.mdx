...
Above the `systemNavBarStyle`, `useDivider` and `opacity` are tied the theme controller in example 5. You can find the implementation in the repo [here](https://github.com/rydmike/flex_color_scheme/blob/master/example/lib/example5/widgets/pages/home_page.dart#L102). Together with the option to remove the status bar scrim, you can have complete control of the look of both the AppBar status bar, and the system navigation bar, as shown below:
...
* There is a temporary workaround implemented, it manages to keep system
  icons from getting the wrong brightness on Android 11 by calling systemChrome twice. These temporary changes will be reverted when the fix for the Flutter issue has reached the stable channel.

You can also use the `FlexColorScheme.themedSystemNavigationBar` to hide the top status icons if you are not using an app bar at all. This can be useful on a splash or onboarding page. Example 5 contains three different examples, each with their own limitations, read more in the example 5 source code comments on how it can be used, here what they look like. The last Example SplashPage 2, would be the ideal version, and it works well on some versions of Android, but it seems to fail on newer ones, so you may prefer 1b instead.
...
It is just not as visible or obvious when using the thin gestures based navigation bar. The example screen shots presenting all the built-in themes in [Scheme reference](/scheme_reference) were produced with fully transparent system navigation bar, using the gesture mode navigation bar.
