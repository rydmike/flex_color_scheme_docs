...
Only code highlights are shown below, the complete code for the example can be found [here](https://github.com/rydmike/flex_color_scheme/tree/master/example/lib/example4_all_themes).
...
Normally for the `FlexKeyColors` properties you would just pass in `true` for the configuration you want. If you for example want to make a seeded `ColorScheme` that uses the hue from all the input colors, and you only want to keep the primary color as a given brand color, then you can use:
