...
Additionally we can activate an option that uses the selected scheme's colors, as input to generate a ColorScheme computed by the Material 3 ColorScheme generation tools. In example 3 we mentioned it being possible to keep primary, secondary and tertiary colors as defined by their inputs, while other are generated. In this example we do can interactively toggle these settings and observe the differences.
...
Only code highlights are shown below, the complete code for the example can be found [here](https://github.com/rydmike/flex_color_scheme/tree/master/example/lib/example4).
...
In this example we use the `ThemeServiceHive` option. The examples are all built using same "example" app. If we use SharedPreferences in more than one of them, they would all use the same storage container and share the settings when you build them locally. By using Hive for most examples, we can change the storage container name for each example. You can of course swap in the SharedPreferences based one here as well, if you want to try it.

Hive is faster though, and accomplishes the same thing and has other features you might find useful even for simple local key-value pair storage, that we are using both SharedPreferences and Hive for in these examples.
...
In it we begin by defining the same colors that we used in example 2 and 3, to be our first custom color. In this case we also assign a custom color to the optional `appBarColor`.
...
The `brightness` parameter in the `FlexSchemeColor.from` factory above is new in version 5.0.0. If `brightness` is specified, the factory computes missing colors that are well suited for a Material 3 based ColorScheme, for the given brightness. If brightness is not specified, then the algorithm is same as before version 5, but with two more colors. Its produced colors fit better for M2 specification, they do also work in M3, they just don't follow the guidelines with respect to the color shades of the main color and its container pair color. When using the `FlexSchemeColor.from` with Material 3 `ColorScheme`, prefer specifying the brightness value for results following the Material 3 design intent.

For our 3rd custom color scheme we define primary and secondary colors, but no container colors, we will not make any dark scheme definitions either, all these missing colors will be computed. The missing color definitions will get computed by the factory `FlexSchemeColor.from` when it creates the `FlexSchemeColor` object. To make our dark colors for this light scheme, we use the method `toDark` further below with the `_myScheme3Light` instance.
...
In this example we keep the list of all our schemes in a convenient to use `FlexSchemeData` object. A **FlexSchemeData** object stores `name` and `description`, plus its matching `FlexSchemeColor` for `light` and `dark` mode color schemes.
...
This list of `FlexSchemeData` is very easy to use as input to our `FlexThemeData` when we want to switch theme colors. You can of course make your own wrappers if you like for the list of input colors. However, both the `FlexSchemeData` and the needed `FlexSchemeColor` classes are immutable classes that come with equality, copyWith and debugFillProperties and via it print too, so you may find them convenient to use.
...
We use `highScaffoldLowSurfaces` where the scaffold background gets a stronger blend than other surfaces. This type is commonly used on web/desktop when you often wrap content on the scaffold, in a Card or other container that has a lighter background.
...
In the app you can then use the popup menu available in the ListTile, to change the active theme. You can choose any of the built-in 40 color schemes, plus the three custom color schemes we added.

When you change theme, you will notice that the active theme color changes are animated by interpolating from the active theme colors, to the new theme colors. This is a very nice standard feature when you modify the theme used by a Flutter Material application. As mentioned earlier in the second example and tutorial, the usage of the `AnimatedBuilder` has nothing to do with this.
...
When the opt-in is true we pass in a `FlexSubThemesData()` constructor, otherwise `null` to not use the sub themes at all.
...
Scroll down in the app to see the theme showcase further below. It presents the theme with common Material UI widgets. You can try this example as a Flutter [web app here](https://rydmike.com/flexcolorscheme/allthemes-v5).
...
In this example we also add 3 boolean values that allows us to define that we want to keep the primary, secondary and/or tertiary colors, as specified by our input key colors, in the generated resulting `ColorScheme`.

When we do so, all the other colors will still be computed by the Material 3 based color generation algorithm. This is a useful feature because for the selected important key colors we may want to keep our custom, typically brand colors, but are OK with using computed and well balanced tones for all other colors in the Material 3 based `ColorScheme`.

This example uses separate controller values for the dark and light theme mode to select which colors we keep. Often we might have a fixed given brand color for light mode, as paper is white. However, many customers may lack a defined brand color suitable for dark mode usage, then you might consider letting the color generation algorithm make one for you using the light mode brand color as the seed key for dark mode.

In this example we for the light theme mode have these "keep" color controller values:
...
A slight custom `StatelessWidget` wrapper of the Flutter `SwitchListTile.adaptive` Widget is used to give us the desired themed platform adaptive look on the switch. But basically the toggle is just the bool controller value with a setter. All the "keep" toggles also contain the additional logic that if we do not use key colors, they get disabled.
...
This setup is a bit complex in this example. The usage based on the the theme controller is used in this example application only in order to be able to demonstrate the differences by using UI input controls that can be toggled to demonstrate the differences.

Normally for the `FlexKeyColors` properties you would just pass in `true` for the configuration you want. If you for example want to make a seeded `ColorScheme` that uses the hue from all the input colors, and you only want to want to keep the primary color as a given brand color, then you could simplicity use:
...
```
Below we can compare the difference of produced `ColorScheme` when using input values as they are, or computed using all main colors as key colors, and then one by one keeping the input value as given. For this example comparison we used the "Blumine, ester blue and saffron mango" theme.
...
You can make complete custom config, or used one of the built-in pre-made custom example configurations. Below we use `FlexTones.vivid` tones constructor, to use a configuration that lets secondary and tertiary palette generation use the chroma as is in their input colors, so the generated tonal palettes will be closer in style to the actual source color.
...
In example 5, the **Themes Playground** you can select between all the different built-in `FlexTones` options, and also see what the actual **Tonal Palettes** look like when you use generated `ColorScheme`s and how each tonal palette color maps to every `ColorScheme` color. It is a handy tool for exploring how the Tonal Palette and mapping to a `ColorScheme` works.
