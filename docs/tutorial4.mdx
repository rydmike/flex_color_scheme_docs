...
generated. In this example we do can interactively toggle these settings
and observe the differences.

As before, we can also turn the opinionated component widget
themes ON or OFF. When it is ON, we can now also adjust the border radius
on all component themes with one single setting.

Only code highlights might be shown below, the complete code for the example can
be found [here](https://github.com/rydmike/flex_color_scheme/tree/master/example/lib/example4).
...
persist settings and persist the theme settings with `ThemeServiceHive`
...
can use in these examples.
  ...
  In this example we chose to use the `ThemeServiceHive` option. The examples
  are all   built using same "example" app. If we use SharedPreferences in more
  than one of them, they would all use the same storage container and share the
  settings when you build them locally. By using Hive for most examples, we can
  change   the storage container name for each example. You can of course swap
  in the SharedPreferences based one here as well, if you want to try it.

  Hive is faster though, and accomplishes the same thing and has other features
  you might find useful even for simple local key-value pair storage, that we
  are using both SharedPreferences and Hive for in these examples.
...
Instead of having our final and const color scheme definition
...
In it we begin by defining the same colors that we used in example 2 and 3,
...
the only required color is the primary color, the other missing colors will be
...
in version 5.0.0. If `brightness` is specified, the factory computes missing
colors that are well suited for a Material 3 based ColorScheme, for the given
...
the guidelines with respect to the color shades of the main color and its
...
all the pre-defined built-in ones, offering them as options that users can
switch to and use if they like.
...
on web/desktop when you often wrap content on the scaffold, in a
Card or other container that has a lighter background.

For demonstration purposes we also define the choice of our `visualDensity` and
`fontFamily` in our static `AppData` class. The values are however the same
as in example 3. You can find the `AppData` class
...
values not used in this example.
...
standard Flutter SDK UI widget, you can find how it is set up for this use case
...
opt in or out of using the component sub-theming via `useSubThemes`. Which is
toggled using a simple ON/OFF switch on the `HomePage`, that allows us to toggle
`useSubThemes` controller value between true and false,
...
otherwise `null` to not use the sub themes at all.
...
`double` value `defaultRadius` controlled by our `ThemeController`.
...
FlexColorScheme then handles the `ShapeBorder` setup based on the passed
...
When we do so, all the other colors will still be computed by the Material 3
based color generation algorithm. This is a useful feature because for the
selected important key colors we may want to keep our custom, typically brand
colors, but are OK with using computed and well balanced tones for all other
colors in the Material 3 based `ColorScheme`.
...
brand color for light mode, as paper is white. However, many customers may
...
the toggle is just the bool controller value with a setter. All the "keep"
toggles also contain the additional logic that if we do not use key colors,
they get disabled.
...
This setup is a bit complex in this example. The usage based on the the
theme controller is used in this example application only
in order to be able to demonstrate the differences by using UI input controls
that can be toggled to demonstrate the differences.
...
`ColorScheme` that uses the hue from all the input colors, and you only want to
...
```
Below we can compare the difference of produced `ColorScheme` when using
input values as they are, or computed using all main colors as key colors,
and then one by one keeping the input value as given. For this example
comparison we used the "Blumine, ester blue and saffron mango" theme.
...
Its concept is also explained and visualized in Themes Playground [guide](/playground#tonal-palettes).
