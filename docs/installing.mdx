...
The Flutter **FlexColorScheme** package is published on [pub.dev](https://pub.dev/packages/flex_color_scheme) and has its open source code in the GitHub [flex_color_scheme](https://github.com/rydmike/flex_color_scheme) repository.
...
The package default example offers a commented code guide on how to use almost every property in FlexColorScheme. The [examples](/examples) chapter shows you how to try it, as well as the five [tutorial](/tutorial) examples.
...
Not as exciting as the images shown on the introduction page. That is because the basic counter app uses very few features and widgets, thus it cannot show the results. The included examples contain demo user interface code. This code is not relevant to using FlexColorScheme, it is only there to present the results.
