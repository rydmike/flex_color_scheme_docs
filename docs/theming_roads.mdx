...
We look at using these different options, or roads to making a ThemeData object:
...
different methods and analyse the results. Yes that's a lot of methods and roads
...
The above enables easy copy/paste of the ColorScheme, but to show the colors for all the
Color values we can use our IDE of choice that can show the colors too.
...
Lots of differences here, let's look at them closer and analyse them one by one.
...
`primaryColorBrightness` in `ThemeData` from its `ColorScheme` colors.analyse
...
ThemeData.from(colorScheme: ColorScheme.fromSeed(...)) method, with
...
We can just copy it and make custom modified version of it. Let's take
a look at doing that.

```dart
// Assuming we have defined our `colorScheme`, we could do this:
ThemeData(
  colorScheme: colorScheme,
  brightness: colorScheme.brightness,
...
since it is less elevated, but otherwise the colors are the same.
...
it will use the default swatch theme based colors.
