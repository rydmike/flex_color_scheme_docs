...
palettes, based on given input colors and then colors from the palettes
are assigned to the ColorScheme.

In the Themes Playground, when you use seed generated ColorSchemes, you can see
all the generated Tonal Palettes.

Below we can see the tonal palettes when using the Flutter SDK default Material
3 ColorScheme tonal palette generation algorithm, based on a single input
key color, it is shown in both light and dark theme. We can see that the
tonal palettes for both light and dark theme are identical, what varies is
which tones are used for each color in light and dark theme mode.

We can for example see that tone 40, from primary tonal palette is being
used for the `ColorScheme` primary color in light theme mode, while tone 80 is
being used in dark theme mode. On the web and desktop app, if you hover with the
mouse over seeded `ColorScheme` color, its source color will be highlighted in the
computed Tonal Palette.

<Tabs
  defaultValue="tonal-light"
  values={[
    {label: 'Tonal Light Primary', value: 'tonal-light'},
    {label: 'Tonal Dark Primary', value: 'tonal-dark'},
  ...
  <TabItem value="tonal-light">
    ...
    caption="Flutter SDK one key color Tonal Palette and primary light color tone mapping"
  ...
  <TabItem value="tonal-dark">
    ...
    caption="Flutter SDK one key color Tonal Palette and primary dark color tone mapping"
  ...
  <TabItem value="tonal-all-light">
    ...
    caption="All key colors Tonal Palette and tertiary container light color tone mapping"
    ...
    caption="All key colors Tonal Palette and tertiary container dark color tone mapping"
...
generation when using primary,
secondary and tertiary colors as seed key input colors, instead of only primary color.
This time as an example showing the tone mapping of the tertiary container color
in light and dark theme mode.

## Flex Tones

Above we saw that even when we use three input color as keys, we do no get so much
...
and it is actually the chroma value from the used key color.

The primary tonal palette also uses the chroma from the key color, but only if
it is higher than 48, if it is lower, 48 is used.
...
rotated +60 from th input key color hue, effectively generating a new hue based
...
create neutral surface colors that contain a hint of te key (primary) color
in the surfaces as well. This is similar in concept to the primary color surface
blends that FlexColorScheme has supported since version 1.0. In the Surface blends
chapter we will se that surface blends can optionally also be used even when
using seed generated ColorSchemes, many of the example above already did do that.

When we used key colors also for the secondary and tertiary tonal palette
generation it above mean we substituted the chroma from primary and secondary as
chroma inputs to their palettes, and no longer relied on the values derived
from the single input primary color.

We did however not do anything the the chroma limits used bay the default Material 3
design Tonal Palette color generation algorithm.

FlexColorScheme, would not be "flex" if it would not let us modify that that too.
We can set custom parameters for the values, that will use the input color chroma
as base, but always lock it to a given value, or we can let it use whatever chroma
it has, or we can say it can use whatever chroma it has, as long as it is over
a certain limit. This is useful because if the chroma is very low in the input color
we basically get a greyscale tonal palette.

With the FlexTones API you can define these limits as desired for the tonal palette
generation. We can do even more than that, we can also modify the mapping of which
tone is used for what color from the generated Tonal Palettes, as long as it is from
the correct source palette. Not all tones are great, but for some cases you can very
well go up or down a tone for a given color in ColorScheme for stronger more contrasty
look or an even more muted one.

The Theme Playground in this version only let's you change FlexTone settings between 5
prebuilt configurations. It is however very simple to make custom ones with the API.
In fact the pre-made ones are mainly intended as examples. You can read more about
