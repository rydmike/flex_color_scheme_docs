...
input colors, instead of only primary color. This time we in the example showing
the tone mapping of the tertiary container color in light and dark theme mode.
...
that is always has a suitable color saturation.
...
and no longer relies on the values derived from the single input primary color.

We did however not do anything to the chroma limits used by the default Material 3
design specification Tonal Palette color generation algorithm parameters.

FlexColorScheme, would not be "flex" if it would not let us modify this too.
We can set custom parameters for the key colors, that define the chroma limits
we will use as limits for the input colors when we generate their tonal palettes.
...
palette colors. With the `FlexTones` API you can define these limits as desired
...
FlexColorScheme also creates surface blends for the Scaffold background and
in some modes, even for dialog backgrounds, treating them as their own
themed surface colors.

In addition to being able to blend in primary color into surfaces, starting in
...
onTertiary and onError, it can be even turned OFF completely, while still
being used by other onColors.
...
principle or not, when your are not using Material 3 seed generate ColorSchemes.
...
generated `ColorScheme`s already include a hint of primary color in all neutral
...
compared to a none seed generated `ColorsScheme`. If you want a pure
Material 3 guide design intent, keep blend level at zero with seed generated
`ColorScheme`s, a bit more is pretty nice too though.

Generated `ColorScheme`s do not use onColor blending. This is because such
...
The impact of this not so visible above. Generally in an application, they
...
colors for each input color in the customizable theme.
