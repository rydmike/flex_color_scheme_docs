...
on, instead of being plain white or black/dark grey. This blending is done more
subtly than the surface colors. For onPrimary, onSecondary, onTertiary
and onError, it can be even turned OFF completely, while still being used by
other onColors.
...
when comparing to none seed generated `ColorsScheme`. If you want a pure
...
The impact of this not so visible above. Generally in the application, they
still create a remarkable subtle toned difference. Most notice it, but might
not be able to immediately explain what the causes the difference in how the
design is perceived.
