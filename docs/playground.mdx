...
<Info>This chapter has not yet been updated to fully cover FlexColorScheme version 7.2. When it has been fully updated, this notice will be removed.</Info>
...
The **FlexColorScheme** package, then when given an API configuration produced by, and copy-pasted from the **Themes Playgrpound**, produces Flutter `ThemeData` object used as a theme in a `MaterialApp`. FlexColorScheme works as fancy `ThemeData` factory, with an often much shorter API for the features it supports.  
...
<Image zoom src="https://raw.githubusercontent.com/rydmike/flex_color_scheme_docs/master/docs/images/playground-info-expand.png" alt="Playground info expand" caption="In-app documentaion and info expand icons"/>


## Many Layouts

The Themes Playground application offers several differnt layouts, some automatic responsive ones, and others that you can switch between manually.

1. One theme topic single page view (phone)
2. Two topics single page view (small tablet)
3. Two topics side-by-side view colors on top (desktop, large tablet)
4. Two topics side-by-side view colors on side (desktop, large tablet)
5. Masonry style grid with collapsible theme topic panels.

The views 1 to 4, are all part of the page view layout mode. This mode switches automatically, depending on screen media size, between layouts 1, 2 and 3 or 4. The choice between 3 and 4 is a manual user preference toggle in the side command bar. There is also a large and compact interface mode, making theme topic panel spacing as well as theme topic and color selection buttons smaller or larger. 

### One theme topic single page view

In this view, the theme topic panel can be swiped horizontally, with eg the thumb, to casually move to the next theme topic panel. When scrolling vertically, the theme topic selection buttons and theme colors selection also scroll away to reveal more contet.

<Image zoom src="https://raw.githubusercontent.com/rydmike/flex_color_scheme_docs/master/docs/images/playground-phone.png" alt="Playground info expand" caption="View 1 - Phone sized single page view"/>

### Two topics single page view

In this view, the single page view shows two theme topic panels side-by-side in a tight layout. This size shows up on typical tablet sizes, on many sizes even in vertical mode. The layout can be toggled between compact and large UI. The right column can show any selected theme topic as well, but the secondary topic selection is done via a popup menu.

<Tabs
  defaultValue="tablet-compact"
  values={[
    {label: 'Tablet (compact)', value: 'tablet-compact'},
    {label: 'Tablet side bar', value: 'tablet-compact-popup'},
    {label: 'Tablet (large)', value: 'tablet-large'},
  ]}> 
  <TabItem value="tablet-compact">
    <Image zoom src="https://raw.githubusercontent.com/rydmike/flex_color_scheme_docs/master/docs/images/playground-tablet-compact.png" alt="playground tablet compact" caption="View 2 - Compact UI tablet size view"/>
  </TabItem>
  <TabItem value="tablet-large">
    <Image zoom src="https://raw.githubusercontent.com/rydmike/flex_color_scheme_docs/master/docs/images/playground-tablet-compact-topicmenu.png" alt="playground tablet compact topic menu" caption="View 2 - Expanded UI tablet size view, with topic popup"/>
  </TabItem>
  <TabItem value="tablet-sidebar">
    <Image zoom src="https://raw.githubusercontent.com/rydmike/flex_color_scheme_docs/master/docs/images/playground-tablet-compact" alt="playground tablet compact" caption="View 2 - Expanded UI tablet size view"/>
  </TabItem>
</Tabs>
...
<Tabs
  ...
  <TabItem value="page">
    <Image zoom src="https://raw.githubusercontent.com/rydmike/flex_color_scheme_docs/master/docs/images/fcs-v5-pg-01-pageview-3.gif?raw=true"
