...
The latest stable version of **Themes Playground** can be used as a [web app here](https://rydmike.com/flexcolorscheme/themesplayground-v7).
...
There are many features that could be considered for a **professional** version of the **Themes Playground** app. For example, using a back-end to store and share multiple theme configurations, privately in a team or publicly. Its features could also be made more designer friendly. Currently, it is mainly focused on being an API configuration tool for developers that use the **FlexColorScheme** package. While it also allows for very flexible and extensive theme configuration, and to interactively try and experiment with different colors and styles on components, it is currently **not** trying to be a designer tool. It does for example not offer any form of import/export compatibility with e.g. popular tools like Figma. Nor will it in its current version. In a future **pro** version of **Themes Playground**, such features may of course be considered.

General export and import of theme configurations could also be supported. Distribution of precompiled executables for Windows and macOS, in addition to the web build, may also be included then. This version of the **Themes Playground** would probably no longer be open source. It would remain free to use at a feature level far exceeding current features, but also offer a paid tier for development teams.
...
<Tabs
  ...
  <TabItem value="page">
    <Image src="https://raw.githubusercontent.com/rydmike/flex_color_scheme_docs/master/docs/images/fcs-v5-pg-01-pageview-3.gif?raw=true"
    caption="Themes Playground - Topic based page view, responsiveness demo."
    alt="page view demo" zoom />
  </TabItem>
  <TabItem value="masonry">
    <Image src="https://raw.githubusercontent.com/rydmike/flex_color_scheme_docs/master/docs/images/fcs-v5-pg-02-masonryview-2.gif?raw=true"
