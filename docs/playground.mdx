...
The **Themes Playground** application started as just a single page, much like example 4, but with a few more controls to modify the actively viewed theme configured by **FlexColorScheme**, it was the final step in the tutorial. It was and still is included as example number 5 in the package repository. The Playground app is built and published as a web app from that example.
...
The challenge with many of the **pro** and **designer** leaning features asked for, are that most of them require a backend, which is not free. Delivering them would require a means to cover the costs. It is not expected that the app would bring any major income, nor would it be priced to aim for that. Theming a Flutter app is probably not a big business opportunity, but it would still need to generate revenue to cover the backend service costs. At this stage, it is unknown if there is enough interest to do that.
