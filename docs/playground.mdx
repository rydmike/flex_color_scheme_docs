...
If that happens, this is potentially the last major release of the **Themes Playground** in its current form and as being included with the **FlexColorScheme** package as "just an example" of how to use the package to build a dynamically themed app. This version will still remain in minor and fix versions of FlexColorScheme v7, but may be removed in a future version 8 of the **FlexColorScheme** package.

There are many features considered for a proper Themes Playground app, like using a back-end to store and share multiple theme configurations privately among teams or publicly. Distribution of precompiled executables for Windows and macOS, in addition to the web build, may also be included then. 
