...
In case you somehow missed it, you can try the **FlexColorScheme Themes Playground** example as a Flutter web app [here](https://rydmike.com/flexcolorscheme/themesplayground-v7-1).
...
You would not normally have this many settings properties in an app that you want to persist locally (or remotely). In those cases, this approach is quite simple and convenient. In this particular case though, well maybe not with this large number of properties. There are **271** individual theming property key-value pairs managed by the same controller. This is a large number of controlled properties, but the setup is simple to make, even if it is quite repetitive.
