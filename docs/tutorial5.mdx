...
Only code highlights are shown below, the complete code for the example can be found [here](https://github.com/rydmike/flex_color_scheme/tree/master/example/lib/example5_themes_playground).
...
You would not normally have this many settings properties in an app that you want to persist locally (or remotely). In those cases this approach is quite simple and convenient. In this particular case though, well maybe not with this amount of props. There are **168** individual property key-value pairs managed by the same controller. Clearly a horrific amount, still the entire setup was pretty simple to make, even if as mentioned quite repetitive.
...
This then avoids the state management debate and there is no need to focus on it in the examples. The usage of the `AnimatedBuilder` is explained a bit more in [tutorial 2 here](/tutorial2#theme-setup).
