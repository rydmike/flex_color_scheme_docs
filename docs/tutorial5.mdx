...
Only code highlights are shown below, the complete code for the example can be found [here](https://github.com/rydmike/flex_color_scheme/tree/master/example/lib/example5).
...
The setup of the application itself is in this case even simpler than we used in example 4. This is because getting the controller driven `ThemeData` configuration for the light and dark theme got so unwieldy large, that we moved them to their own ThemeData functions. This gives a very compact `StatelessWidget` as our `DemoApp` that uses `MaterialApp` and the `FlexColorScheme` produced `ThemeData`.
...
Well that was not a lot, it only calls `flexColorSchemeLight` passing along our `ThemeController`, the `controller` to the `flexColorSchemeLight` function, that returns a `FlexColorScheme` object, and it then returns its `ThemeData` using the `toTheme` method. This is then used as `ThemeData` by the `theme` property in our `MaterialApp`. Theme done! Well almost, let's open up the details and see why we did it like this.
 ...
 We do it this way and not directly, or by setting it up via `FlexThemeData.light`,  because for the none `FlexColorScheme` using case, when we create a standard  Flutter SDK theme with `ThemeData.from` a `ColorScheme`, we will use the same `flexColorSchemeLight` function to return the `ColorScheme` it represents with its `toScheme` method.
...
For presentation reasons the complete code for the `flexColorSchemeLight` function has be cut considerably. You can find its complete code in the repo [here](https://github.com/rydmike/flex_color_scheme/blob/master/example/lib/example5/theme/flex_theme_light.dart#L22).
...
Let's take a look at what we do for the case when we turn OFF the usage of `FlexColorScheme`. In the [Roads to ThemeData](theming_roads), we demonstrated the [`ThemeData.from`](/theming_roads#6-tdfrom-colorscheme) a `ColorScheme` option. We use that here too.

What we want to do is to produce `ThemeData` that will use all relevant `ColorScheme` generation selection and settings from the `FlexColorScheme` setup. Use this `ColorScheme` as input with `ThemeData.from` factory, so we can get a Flutter's default `ThemeData` using the identical `ColorScheme`.
...
In case you somehow missed it, you can try the **FlexColorScheme Themes Playground** example as a Flutter [web app here](https://rydmike.com/flexcolorscheme/themesplayground-v6).
...
By resizing the window on the web version, you can see the responsive designs used. In the Themes Playground guide you can see some screens recordings [showing](https://docs.flexcolorscheme.com/playground#two-layouts) it in action too.

## ThemeController

Before we wrap up, a few more words about the `ThemeController` and used persistence model. This is not relevant for **FlexColorScheme** usage, but might be of geneal interest.
...
This setup on purpose demonstrates persisting each theme property value as its own string key and value pair. With this amount of values, bundling them all up in a data class and persisting them all as one big serialized JSON string, with just one settings key, would certainly be more convenient. This setup is however much more file resource efficient and gives us faster saves of persisted slider and color picker values, that can be dragged around quickly in the UI.

Some of the theme changes can be a bit taxing calculation wise. While some changes that appear to be lagging behind, do so only due to the theme change animation. The `MaterialApp` always lerp animates a theme change from all its previous `ThemeData` property values, to its new `ThemeData` property values. So when you drag sliders to change border radius, this triggers an animation from a complete `ThemeData` based on the previous slider value to the next one, based on the new slider value, for every rapid slider value change.
...
In cases where we use nullable properties that also needs to be persisted, we obviously skip the null check in the setter above. The logic ensures we do not do any work not needed.
...
Typically you would not have this many settings properties in an app that you want to persist locally (or remotely). In those cases this approach is quite simple and convenient. In this particular case though, well maybe not with this amount of props. There are **136** individual property key-value pairs managed by the same controller. Clearly a horrific amount, still the entire setup was pretty simple to make, even if as mentioned quite repetitive.

It works surprisingly well despite the large amount of controller properties, even if the values all share the same listener that rebuilds the app when something changes. This is of course because every property in the controller is of the nature that if we change it, then something in `ThemeData` changed, and we need to rebuild the entire app with the new `ThemeData`.

This does not mean that Flutter always redraws every widget when we change something, if it is local to just one widget type, like input decorator used only in one panel, we can with rainbow paints, even on the big masonry grid view, observe that often only it is being redrawn. Naturally if we change theme colors, everything needs to be redrawn with the new colors. The nature of this app being what it is, this is of course happens a lot.

### Persisting "null" Choice

For many theme properties "null" represents the default undefined theming behavior. In our settings that is also a deliberate choice and option that we may choose and want to persist. Whereas a missing key in our persisted key-value pair, only means we have no value for it in it yet, and we should get the app default value for the property. Which is available as const value in the app via `Store.default"Property"` values. The default might not be null in the app defaults, but it may also be so for some properties.
...
The interactive theme manipulation done in the Themes Playground is very fancy, but the app itself is still quite simple and "smallish". So I thought this case would be suitable to still implement using the simple Flutter skeleton architecture using `ChangeNotifier` and a listenable builder in the form of the `AnimatedBuilder`.

This then avoids the state management debate and there is no need to focusing on it in these examples. The usage of the `AnimatedBuilder` is explained a bit more in [tutorial 2 here](/tutorial2#theme-setup).

Could Themes Playground be made more efficient using Provider, Riverpod, Flutter Bloc, MobX etc.?

Yes most likely, but things can also be improved with the current used solution, by breaking things up in even smaller widgets, as well as taking out a few properties that I due to laziness tucked into the same controller, even if they were not really related to the theme being changed, they could and should be given their own controller, or maybe an inherited widget to mix it up a bit.

If the Themes Playground app needs to get even wilder and fancier than it already is now. I might split it out to its own repo, and migrate it to eg. Riverpod, but probably not in the near future.

## More Topics

In future updates to this tutorial, or as separate blog posts, I could if it is of interest go a bit deeper into general interest topics in the **Themes Playground** app on how certain details were made, like:
...
* Maybe dive deeper into `FlexColorScheme` and its `FlexCorePalette` extension of Material Color Utilities `CorePalette`, as well as its custom and private `_Scheme` mid-layer to adjust tone palette mapping and create a `ColorScheme.fromSeeds`, instead of just a `ColorScheme.fromSeed`.

Drop a note in the [repo discussion](https://github.com/rydmike/flex_color_scheme/discussions) if there is something in the example apps or in FlexColorScheme in general that you would like more in-depth info on. I might add a chapter about it here if suitable, or in a separate blog post.
