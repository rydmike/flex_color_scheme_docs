...
### Beware: `SchemeColor` new values and order
...
break anything in major ways, other than possibly loading local storage of
...
to `secondary`. This means that if your application used those
...
- or used them via a custom `FlexSchemeColors` in the `colors` property in
`FlexColorScheme`.
...
If you upgrade package version to 5.0.0 and have used custom color schemes,
you will thus find your custom variant colors on the corresponding new
container colors.

Be aware though that past variant color shades are not necessarily a great
...
to do even if you are not using FlexColorScheme.
