...
FlexColorScheme version 6 has some minor default style changes that are breaking with previous used defaults. It has also removed all members that were deprecated in version 5.1 or earlier byt still present. To make migration easier, make sure in version 5.1 that you are not using any of its deprecated APIs. Version 5.1 will still tell you witch members are deprecated and what you should replace them with. In this version those members are totally removed and there is no longer any deprecation guidance on them.

**Changes made to increase Material 3 style compliance by default**

All noticeable style changes concern FlexColorScheme defaults when opting in on Material 3. Some widgets that in the Flutter SDK have received correct Material 3 design, now also in FlexColorScheme when opting in on Material 3, defaults to their correct Material 3 default styles. Previously they may have used a Material 2 style that was Material 3 inspired, but not entirely in-line with Material 3.
...
```
### FAB Use Global Border Radius

The `FlexSubThemesData.fabUseShape` opinionated component theme style default was changed from `true` to `false`, this breaks previous default style where FAB also by default got border radius from the global FCS setting. The opinionated style change was done to be able use a style that by default matches M3 style when `ThemeData.useMaterial3` is `true`. If you had kept `FlexSubThemesData.fabUseShape` unspecified and relied on default value in a previous version, you must set it to `true` to get the same result as before.
