...
Version 7.0.0-dev.2 is a not yet fully tested development, or beta version of **FlexColorScheme** 7.0.0. It is now using Flutter stable 3.7.0.

**Why version 7.0.0? Are there breaking APIs?**

There are actually no breaking APIs in version 7 compared to version 6. You can upgrade to the new version without any changes. However, some past styles have been slightly tuned to align things better with M3. Some tests broke due to this, so the major version has been bumped.
...
The new menus are one of my personal favorites in the new Flutter version. The same underlying component is used for the menu in `DropDownMenu`, `MenuBar` and `MenuAnchor`. We now have a lovely dropdown menu in Flutter. The `MenuBar` is great for desktop apps, it supports nested menus and keyboard shortcuts deep into the nested menus. The `MenuAnchor` is like one menu from a `MenuBar` that you can attach to any widget. Common use case could be right- or control-click context menus. For the new menus, FCS7 offers theming of menu part. More quick styling options may be added in later versions.

The `DropdownMenu` has a `TextField` input part, it follows the general `TextField` input decoration you defined in FCS. In version 7 there are more quick config options available for the `TextField` and its `InputDecoratior`. Another neat synergy with menus is that for the menu options in the bar and the menu selections, the same `ButtonStyle` button that the normal Material button use is used as their building block. If you are familiar with raw custom theming the `ButtonStyle` Material buttons, with their complex but versatile MaterialState, you already know how to theme menu selection options. FCS may offer some simpler quick styling of the menu selections in later versions. 
...
The FlexColorscheme documentation site has otherwise not been updated with any more information about FCS7. Updates to the doc site will follow in stages after the stable version of FCS is released.

<Info>For full details of what is new check the [changelog for version 7.0.0-dev.2](https://pub.dev/packages/flex_color_scheme/versions/7.0.0-dev.2/changelog).</Info>
