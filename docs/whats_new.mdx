...
The new menus in Flutter 3.7 are a great additions to Flutter's component portfolio. The same underlying component is used for the menu in `DropDownMenu`, `MenuBar` and `MenuAnchor`. The `DropDownMenu` is a very capable dropdown menu, now available in Flutter SDK out-of-the box. The `MenuBar` is great for desktop apps. It supports nested menus and keyboard shortcuts deep into the nested menus. The `MenuAnchor` is like one menu from a `MenuBar`, that you can attach to any widget. A common use case is e.g. right- or control-click, based context menus. For the new menus, FlexColorScheme offers extensive theming customizations. There are still some styling limitations due to a few lacking features and bugs in Flutter, but a lot can be done to customize them already now. 
