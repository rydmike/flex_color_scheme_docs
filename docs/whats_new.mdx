...
To migrate to the new version, see the summary in the [**Migration Guide**](/migrating). For a detailed log of all changes, please see [CHANGELOG on pub.dev](https://pub.dev/packages/flex_color_scheme/versions/8.0.0-dev.2/changelog) on pub.

Compared to `8.0.0-dev.1` the `8.0.0-dev.2` package release contains a few minor bug fixes discovered during QA testing and test updates for the package.

The **Themes Playground** app got two **major** new features in `8.0.0-dev.2`. 
 1. It can now export a Playground configuration to a JSON file. You can later import this JSON file to restore the Playground to the same state as when you exported it. This **Playground** feature was a nice contribution by GitHub user [@akiller](https://github.com/akiller) in [!PR 257](https://github.com/rydmike/flex_color_scheme/pull/257), thank you! This contributed feature got enhanced with more error handling and a slightly refined UI to make it production ready. It can now be tried for the first time in the 8.0.0-dev.2 release and build.
 2. You can also convert an exported JSON settings configuration to a shareable link. This link can be shared with others. When they open it, the **Themes Playground** will reconfigure itself to the state it had when the shared settings were exported and converted to a link. 

### Can I use this V8.0.0-dev.2 release now?

Yes you can. There are no planned API changes to the package before the 8.0.0 stable release. Quality assurance testing is far along, and the package has now passed all critical tests. Some minor test updates remain. Stable version 8.0.0 will be released without 100% test coverage. It will be around 95% at its first stable release and more tests will be added in fix releases to bring coverage back to 100%. The documentation site will also be gradually updated and finalized after the 8.0.0 stable release.
