...
* Due to Flutter SDK
  ...
  * There is a temporary workaround implemented, it attempts to keep system
    icons from getting the wrong brightness on Android 11. It may not always work.

  The temporary changes will be reverted when a fix for the Flutter issue
  has reached the stable channel.


 * ShapeBorder on input decorator `UnderlineInputBorder` has faulty equality
