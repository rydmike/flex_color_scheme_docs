...
## Flutter SDK: Chips in Material 3 mode do not follow the specification.

Chip does not conform to Material3 design spec when useMaterial3 is enabled. See [issue #115364](https://github.com/flutter/flutter/issues/115364), no corrective actions started yet (Nov 18, 2022).

## Flutter SDK: NavigationBar ink ripple does not follow M3 spec

The `NavigationBar` has a Material 2 type ink effect when opting in on Material 3. This is not according to Material design specification. See issues [113870](https://github.com/flutter/flutter/issues/113870) and [#114618](https://github.com/flutter/flutter/issues/114618).specification

There is an open (Nov 18, 2022) [PR](https://github.com/flutter/flutter/pull/115499) in the master channels that aims to fixes this and related issues.

## TextField in Material 3 mode has wrong hovered and focused styles
...
The [PR #113776](https://github.com/flutter/flutter/pull/113776) in master channel fixes the issue. This issue only occurred in the master channel, because M3 mode support for `TextField` does not exist in the Flutter stable channel. Hopefully, this issue will never regress to the stable channel. We will see when next stable version after Flutter 3.3 is released.
...
A fix [PR #110870](https://github.com/flutter/flutter/pull/110870) for this issue now exists in master channel, but it is not yet available in Flutter 3.3.
...
When defining `foregroundColor` for the new Material 3 style `SliverAppBar.large` or `SliverAppBar.medium` either via an `AppBarTheme` or their properties, the defined color is not used by the app bar, see issue[#110951](https://github.com/flutter/flutter/issues/110951). This issue had when last checked (Nov 18, 2022) no known corrective actions in progress in the master channel.
