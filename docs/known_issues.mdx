...
Currently, all known issues **impacting FlexColorScheme** are rooted in **Flutter SDK issues** and typically cannot be fixed in the package. All known issues are being tracked, and status changes are updated below. A few of the below listed Flutter SDK theming issues have workarounds included in FlexColorScheme. Such workaround cases are also mentioned in the issues below. 
...
<Error>Applies only to Flutter 3.7.</Error>

The background color of the `BottomAppBar` cannot be changed when enabling Material 3 in Flutter. This applies to using its color property and theme and any other mean of trying to change its background color. This is bug in Flutter 3.7, it has a fix [PR #117082](https://github.com/flutter/flutter/pull/117082) in master channel from December 16 2022, that did not get included in the Flutter 3.7 Jan 25, 2023 release. The issue is also reported in the FlexColorScheme repo here [FCS #115](https://github.com/rydmike/flex_color_scheme/issues/115).

## Flutter SDK: NavigationBar ink ripple moves with height changes (Regression)

<Error>Applies only to Flutter 3.7.</Error>

If you change the `NavigationBar` from M3 default value of 80dp, the ink level moves up or down with same amount as the devation from the default 80dp height. See issue report and illustration here [FCS #114](https://github.com/rydmike/flex_color_scheme/issues/114) as well as in Flutter SDK issue [#117420](https://github.com/flutter/flutter/issues/117420). The issue is caused by a regression in Flutter 3.7. It does not exist in Flutter 3.3 and earlier. It is also fixed in the current master channel via [PR 117473](https://github.com/flutter/flutter/pull/117473). This fix has now been cherry-picked (CP) via [# 119553](https://github.com/flutter/flutter/pull/119553) to be included in a future hot-fix for Flutter 3.7, hopefully it will arrive in the first hotfix 3.7.1.

## Flutter SDK: Known minor M3 specification deviations

<Error>Applies to Flutter 3.7 and earlier, if widget with spec a issue supported M3 in earlier versions.</Error>

The following M3 themes in Flutter 3.7 have minor deviation errors from their Material 3 specification.

- TimePicker clock-dial uses wrong spec color and its M3 web spec has a mistake. See issue [#118657](https://github.com/flutter/flutter/issues/118657). FCS7 includes a theme fix for this.
- BottomSheet has incorrectly has a shadow. See issue [#118244](https://github.com/flutter/flutter/issues/118244).
- BottomAppBar does not match M3 spec, it incorrectly has a shadow. See issue [#118150](https://github.com/flutter/flutter/issues/118150).
- Wrong selected and disabled InputChip color, see issue [#115826](https://github.com/flutter/flutter/issues/115826).
- Missing state elevations on Chips. See issue  [#115825](https://github.com/flutter/flutter/issues/115825).
- No Ink Splash on Actionable Chips when using color on them. See issue [#115824](https://github.com/flutter/flutter/issues/115824).
...
The `Chip` does not conform to Material3 design spec when `useMaterial3` is enabled. See umbrella issue [#115364](https://github.com/flutter/flutter/issues/115364) for details. No corrective actions yet (Nov 18, 2022). 

## Flutter SDK: Cannot set foreground color on SliverAppBar medium and large

<Error>Applies to Flutter 3.7 and earlier.</Error>

When defining `foregroundColor` for the new Material 3 style `SliverAppBar.large` or `SliverAppBar.medium` either via an `AppBarTheme` or their properties, the defined color is not getting applied to the `AppBar`as it should. See issue [#110951](https://github.com/flutter/flutter/issues/110951). This issue has been fixed in the master channel via [PR #118322](https://github.com/flutter/flutter/pull/118322), but it has not landed in stable 3.7. It would be nice to get a cherry-pick (CP) for a hotfix to Flutter 3.7 for this issue, but it has not been picked yet.
...
<Error>Current status to be re-checked. Partial fix may exist even if the issue is still open.</Error>
