...
based `ThemeData`. You should always use null fallback values if you access ThemeData sub-themes and
their properties in `Theme.of(context)` that are null in default `ThemeData` or its component themes.
...
implementation details are only considered style breaking if they produce a theme that is visibly different from
past behavior.
...
In some cases, typically for some core related "legacy" component theme cases,
the themes are created directly in the `toTheme` method.
However, in most cases it uses separate static sub-theme helper functions
