...
FlexColorScheme assigns its defined and computed colors to the `ThemeData.colorScheme` to
ensure that its colors are applied to all widgets that adhere to this newer standard.
Then it also defines all still existing legacy direct colors in `ThemeData`, that some Flutter Widgets
...
from the `FlexSubThemes` class are used. The component themes that are defined
...
FlexColorScheme at all.

The `FlexSubThemesData` configuration has
...
styling on `ToggleButtons` when it comes to hover, press, selected and focus.
...
the `ButtonThemeData` will also soon be deprecated and later removed in Flutter.
