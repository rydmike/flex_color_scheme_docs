...
The defaults for these mostly still Flutter Material 2 based using component themes, draw inspiration from Material 3. Their are inspired by the [Material 3 Design Guide](https://m3.material.io/), within reasonable limits of current Flutter Material 2 based theming capabilities.
...
component border radius theming for now:
...
* `Drawer` should have have rounding on shown side edge, it is in recent
  version possible to assign a Shape in the drawer theme. But
  ...
  behavior modifying it. A default behavior in SDK can implement it by looking
  up if Drawer is being used in Scaffold as Drawer or EndDrawer, but support
