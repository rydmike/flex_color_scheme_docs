...
**TODO: Updated to V5, but needs review, it is a bit messy still**

This chapter explains what `ThemeData` modifications `FlexColorScheme` core does
wen you use it without opting in on the opinionated component themes.
...
many additional custom theming.  It does of course define a `ColorScheme` that it uses in its
...
It also uses the [Material Color Utilities](https://pub.dev/packages/material_color_utilities)
from the Google Material theme that Flutter SDK also uses. It extends the
`CorePalette` to be able to tweak the `ColorScheme` generation parameters in it,
and uses its own customizable Tone to Scheme mapping.
...
themes, it is not as much. But a few component themes are also created by the
core version, tey are listed below.
...
theme and its properties. The widget colors for such cases are then typically
defined by `ThemeData.of(context).colorScheme`.
...
by now, but there are still be some components in Flutter remaining that do not fully adhere to this design.

FlexColorScheme first sets its scheme to the `ThemeData.colorScheme` to ensure that its
colors are applied to all widgets that adhere to this newer standard.

Then it also sets all still existing legacy direct colors in `ThemeData`, that some Flutter Widgets
still use. These `ThemeData` colors are all set to use `ColorScheme` appropriate or from it
...
did not specify component themes.

Even though some component themes currently are not null, and have some none null properties in FlexColorScheme
...
needed in FlexColorScheme, such default force built-in sub-theme definitions may eventually be
removed, **but not without due warning**.
...
Another similar change was the deprecation of `ThemeData.accentColor`.

### Currently Modified and None Null Component Themes with Core
...
enables you to get more styled and themed widgets automatically. You
can customize the styles further via
...
The defaults for these still Flutter Material 2 based theming features,
draw inspiration from Material 3, and their defaults follow the
...
Flutter. Since Material 3 will become the new normal, FlexColorScheme will
potentially make some opinionated design choices concerning them in future
versions, this remains to be seen.
...
Natural `FlexColorScheme` would not be "flex" if the
[`FlexSubThemesData`](https://pub.dev/documentation/flex_color_scheme/latest/flex_color_scheme/FlexSubThemesData-class.html)
...
A common use case for `FlexSubThemes` and the `FlexSubThemesData` is easy to
...
or [BorderRadiusGeometry]. This can be done with a single property the
...
In some simple cases, for some core related legacy component theme cases,
the themes are created directly in the `toTheme` method.
But in most cases it uses separate static sub-theme helper functions
from the `FlexSubThemes` class. The component themes that are only defined
