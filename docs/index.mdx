...
<Info>**FlexColorScheme** package dev release 8.0.0-dev.1 has been published. This is a BIG update with breaking changes and many new features. It has its own dev pre-release of the [**Themes Playground**](https://rydmike.com/flexcolorscheme/themesplayground-dev) as well. No new API changes are planned for the stable 8.0.0 release from this dev.1 release. The package is currently undergoing QA testing and documentation updates prior to its stable release. For more information, checkout the [**What's new**](/whats_new) chapter and the [Migration guide](/migrating) summary, or read the full detailed [Changelog](https://pub.dev/packages/flex_color_scheme/versions/8.0.0-dev.1/changelog) on pub. We would appreciate any feedback about found issues from early adopters.</Info>

<Warning>Flutter 3.22.0 introduced major breaking style and color mapping changes as a part of its new ColorScheme. FlexColorScheme v7.3.1 and earlier versions are designed to work with Flutter 3.19 and earlier. Material-3 theming changes in Flutter 3.22 are more significant than what appears on the surface. The release silently broke **249** of its own existing internal tests, [see here](https://github.com/flutter/flutter/issues/130135#issuecomment-2308522312). An updated version of FlexColorScheme (V8), that is fully compatible with the new ColorScheme, new color deprecations and component color mappings in Flutter 3.22 and later, is now available as dev release. Using FlexColorScheme 7.3.1 and earlier versions with Flutter 3.22 and later versions, may result in unexpected now legacy surface color results. This is also true if you used a very customized ThemeData design without even using FlexColorScheme. Using FlexColorScheme 7.3.1 does technically work and build with Flutter 3.22 and later, but it does not do all the new surface colors and all their default color mappings in accordance to the latest Material-3 design used in Flutter 3.22 and later.</Warning>
