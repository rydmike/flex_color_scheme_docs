...
<Info>**FlexColorScheme** package dev release 8.0.0-dev.1 has been published. It has its own dev pre-release of the [**Themes Playground**](https://rydmike.com/flexcolorscheme/themesplayground-dev) as well. No new API changes are planned for the stable 8.0.0 release. The package is undergoing QA testing and documentation updates before its stable release. For more information see [**What's New**](/whats_new) or read the detailed [CHANGELOG](https://pub.dev/packages/flex_color_scheme/versions/8.0.0-dev.1/changelog) on pub.</Info>
