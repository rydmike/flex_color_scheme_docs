...
<Warning>**Flutter 3.22** contains major breaking style and color mapping changes via its new ColorScheme. FlexColorScheme 7.3.1 and earlier versions are only designed to work with Flutter 3.19 and earlier. An updated version of FlexColorScheme (8.0.0) is in progress. Changes in Flutter 3.22 are much more significant than what appears on the surface. It will take some time before a fully compatible version is ready for release. FlexColorScheme 7.3.1 and earlier versions may result in unexpected surface colors if used with Flutter 3.22. This also applies to if you use very customized ThemeData without even using FlexColorScheme. Please be patient while we work on the updated and compatible version.</Warning>
