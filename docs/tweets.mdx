...
This tweet thread is the latest one. It uses a fictive theming use-case story to demonstrate more advance feature in the current version of FlexColorScheme and Themes Playground. Like how to use custom brand color, with seed generation using brighter color generation, while also locking down your brand colors to the fixed given colors.

It also demonstrates how to use the in version 7.0 introduced platform adaptive theming properties, to create a theme that automatically looks less Material-3 opinionated on and more platform-agnostic on other platforms than Android. 
...
Some sneak peeks into work in progress, selecting a shape type for the corner radius to be something else than circular. Most notably "squircle" shaped, which is used by Apple in HIG.
