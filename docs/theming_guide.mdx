...
This chapter gives you an introduction to application theming in Flutter.
...
and also explain some choices made in FlexColorScheme.

This guide does not talk about how to make beautiful designs, its focus is on
the technicalities of how to actually define your `ThemeData` and make it
work for you. In the [Roads to ThemeData](/theming_roads) chapter we look at
...
To give you a starting point, with many easy to configure options, that should
get you close to where you want to be with your theme, or maybe even fully there.

## Why Theme an App?

You should strive to make the built-in widgets in Flutter default to looking
as close as you can to the look and design you want them to have in your
...
`colorScheme` based color values, if they used used those colors. Currently this
...
when you change theme, since they are still themed color values, but not just defined
...
component themes should use on the its component. If you make completely
custom component themes, you of course use whatever colors you prefer in them.
...
TextTheme TextStyle is used as default style by which Flutter Widget.
...
is exactly what the Material 3 styles did. This was a good move since pretty much
...
and it is not well documented anywhere where, other than in the above article.
If you need to know, you can use it, or do a search of a style in the SDK source
code to find out where they are used.

If you modify the text styles, and don't know exactly how they are used where,
you may get unforeseen results on some widgets due to the change. Adjusting them
a few point sizes up or down can be OK. However, some widgets, e.g. ListTile uses the color
