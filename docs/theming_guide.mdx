...
This chapter gives you general guidance to application theming in Flutter. It is not about using **FlexColorScheme**, even if it on occasion mentions it. When it does, it mostly explains some choices made in FlexColorScheme. 
...
Starting from version 5.1.0 FlexColorScheme support the Theme Extensions API directly as well. It is just a convenience pass through to the `ThemeData` API, so you don't have to add it with and extra ``opyWith`. You use it exactly the same way as the extensions in ThemeData. The [default example](/examples#default-example---hot-reload-playground) includes a small example on how to use it.
