...
It is not about using `FlexColorScheme` even if it on occasion
...
This guide does not talk about how to make beautiful designs, its focus is on
the technicalities of how to actually make your `ThemeData` and make it
work for you. In the [Ways to ThemeData](/theming_roads) chapter we look at
...
closer to where you want to be or maybe even fully there.
...
application. You should do this by defining an application theme that brings
...
component themes should use. If you make completely custom component themes, you
can however use whatever colors you like in them.
...
be able to check in the source what the new defaults really are.

As a future update for this guide, I plan to add a cross reference table
...
Another frequent thing I frequently look up in Flutter SDK code is the
TextTheme and which text style is used where by what widget. The only place
to find out is also in the Flutter source code. I might as well add cross
reference table for it as well. I also plan to extend this guide with some
pointers and tips on custom text theme and text styles for Flutter apps.

## Custom Themes?

You can make custom inherited themes for your custom widgets as well, you can
even extend Flutter's theme. We won't be going into that in this guide in this
...
but its principles are still the same.


## Custom TextThemes?

The article also contains an older cross reference of what TextTheme TextStyle
is used as default style by which Flutter Widget. Since the article is a bit aged,
some of them might have changed, but I don't think so. There are however some
newer widgets that are not covered, simply because they did not exist when the article
was written. For the same reason it also does not contain a cross reference of the new
M3 style with the past M2 styles.

You can find an image comparing the new Material 3, to the Material 2 text styles
in this Flutter issue [#89853](https://github.com/flutter/flutter/issues/89853).
Here is the same image borrowed from the above source:
...
generally not used much, they can mostly be quite freely modified without
impacting component defaults in unforeseen ways. As can be seen above, this
is actually what Material 3 styles did. This was a good move since pretty much
all of the M2 heading text styles were too big by default to be very useful.

The smaller TextStyles in the TextTheme are used in the SDK all over the place,
and it is not well documented anywhere where. You can use the earlier cross
reference table, or do a search of a style in the SDK source code to find
out where they are used.

If you modify the text styles, and don't know exactly how they are used where,
you may get unforeseen results on some widgets. Adjusting them a few points
up or down can be OK. However, some widgets, e.g. ListTile uses the color and its
opacity from one TextStyle, it borrows it and applies it to another TextStyle is uses
from the TextTheme. This is done to create less impactful default subtitle text
...
change the source TextStyle's color opacity it uses for this, your ListTiles would
...
On the smaller and mid sized TextStyles in TextTheme, only make minor size adjustments
and letter spacing tweaks. Don't do anything too radical with it, then your app's
components may still look fine.

For bigger changes, add your custom TextStyle to the component theme levels
...
Then modify the TextStyle(s) it uses for your global app TextTheme, to give it the desired
themed TextStyle, but don't do that. If you do, multiple other components will be impacted.

The `AppBarTheme` has a `titleTextStyle` and `toolbarTextStyle`, define these `TextStyle`s
...
component theme has and uses a component level TextStyles. At least check for it
...
component themes. It only includes `textTheme` and `fontFamily` pass-on to `ThemeData` for the
global app TextThemes. It also has a custom "m3like" TextTheme that it for now uses when you opt-in
on using the not yet merged m3 Typography (sizes and geometry). This text theme will be replaced
by using the actual M3 TextTheme and its geometry once it lands in Flutter stable. This is the
[PR 97829](https://github.com/flutter/flutter/pull/97829) that needs to land in stable for
that to happen.

FlexColorScheme could perhaps in some later version add som TextStyle configuration options to
some of its opinionated components themes too. At least where it is frequently used.
...
still supported in order not to break past behavior.
...
Material UI Widgets. To get it completely covered, you still also have to assign
...
of color properties never get any `ColorScheme` based values assigned to
...
In the chapter [Ways to ThemeData](/theming_roads) we will look at all the
...
to be modified or added, in order to replicate past **FlexColorScheme** theming
behavior, when its corresponding color property is removed from `ThemeData`.
