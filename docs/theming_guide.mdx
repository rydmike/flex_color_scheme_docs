...
This chapter gives you general guidance to application theming in Flutter. It is not about using **FlexColorScheme**, even if it on occasion mentions it. When it does, it mostly explains theming choices made in FlexColorScheme. 
...
The `ColorScheme` class is a data class that in **Flutter** currently holds 30 `Color` properties. Flutter's built-in **Material** UI components use these colors in predefined ways. How each Material component and corresponding Flutter UI widget use the colors in the `ColorScheme`, is specified in the components part of the [Material 3 Guide](https://m3.material.io/components). Likewise, the older [Material 2 design](https://m2.material.io/components) contains specification for how Material 2 components uses these color properties. The property name for the used `ColorScheme` in `ThemeData` is `colorScheme`. 
...
<Info>Many Flutter software teams refer to using custom wrappers for Flutter Material Widgets that use values defined in other style-defining classes as defaults, as their custom in-house design language or style. 

They still use Material design and widgets, may partially also use Material theming. This is really only using new defaults for Material, not really a new design language, it is just a use case and design based on Material Design.</Info>
