...
If you prefer to watch a live recorded talk about these topics, there is one from Flutter Nordics meetup May 17th, 2022. You can find links to the recording of my live streamed talk [Flutter App Theming here](https://rydmike.com/blog_m3_talk). In the talk I present many of the same topics as those that are explained in this chapter and in the chapter ["Roads to ThemeData"](/theming_roads).
...
Starting from version 5.1.0 FlexColorScheme supports the Theme Extensions API directly as well. It is a convenience pass-through to the `ThemeData` API so you don't have to add it with an extra `copyWith`. You use it exactly the same way as the extensions in `ThemeData`. The [default example](/examples#theme-extensions) includes a small demonstration on how to use it.
