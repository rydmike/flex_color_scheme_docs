...
In the chapter [Roads to ThemeData](/theming_roads) we will look at all the different ways to define a `ThemeData` object. We will then see different results, like the ones below. Depending on how we create our `ThemeData`, the results can vary a lot, even if we use the same input colors.
