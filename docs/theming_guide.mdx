...
This nice article also contains a cross reference of what TextTheme TextStyle
is used as default style by Flutter Widgets. Since the article is a bit aged
some of them might have changed, but I don't think so. There are however some
newer widgets that are not covered, since they did not exist when the article
was written. For the same reason it also does not cross reference the new
M3 style names with the M2 names. These are things I plan to revisit and
add as reference section to this documentation site.
