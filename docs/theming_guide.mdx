...
The `ThemeData` object is what defines how your application looks and even to a 
degree how it behaves in Flutter. The topic ["Defining ThemeData"](/theming_guide#defining-themedata) and
and the entire chapter ["Roads to ThemeData"](/theming_roads) looks at it in 
even more details.

The `ColorScheme` class is a simple data class that holds 27 `Color` properties
the define colors values that Flutter uses on built-in components by default 
in your `ThemeData`. The `ColorScheme` is stored as a property `colorScheme` in
`ThemeData. The `ColorScheme` used to be a a lot simpler prior to Flutter 2.10. 
To get an understanding of how the color system that uses the colors in your 
`ColorScheme` are designed and intended to be used, please refer to the
[Material 3 guide](https://m3.material.io/styles/color/overview).

This Tweet 15 slide series, is also a quick intro to the wondrous world of
