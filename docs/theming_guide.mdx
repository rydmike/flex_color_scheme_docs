...
You should strive to make the built-in widgets in Flutter look as close as you can to the look and design you want them to have in your application. You should do this by defining an application theme that gets you as close as possible. By doing so, you are using Flutter's theming capabilities to work for you, and not against you. When you do this, most of your application's design will fall into place almost automagically.
...
This is a good question. In Material 2, they follow the [Material 2 design guide](https://material.io/components) defaults, and when you enable Material 3, they follow the [Material 3 design](https://m3.material.io/) guide defaults. Well the widgets that have been migrated to support Material 3 in Flutter 3.3 do so. There are still many widgets that do not yet implement Material 3 in Flutter 3.3.
...
It is worth noticing that Flutter widgets that have been migrated to support Material 3, also typically have had their default un-themed styles and behaviour implementation updated for Material 2 as well. You can now typically at the end of each widget, find two component theme extension for the widget in question, one that extends the component theme with Material 2 default and another with Material 3 default. Depending on if you have opted in on using Material 3 by setting `useMaterial3` to true in `ThemeData`, the widget then uses the default from corresponding class. It is very a clean and nice new way to be able to find the UI widget default values this way.
...
A new and exiting feature that arrived with Flutter 3.0.0 is **Theme Extensions**. This new feature is nicely presented in this FlutterDev YouTube video, released May 3, 2022:

<YouTube id="8-szcYzFVao" />

_Flutter Theme Extensions_

This feature finally solves the limitation of not being able to easily hook into the Flutter theming engine and extend it with properties your custom widgets need. A very simple use case is just adding more colors to the theme that your application needs, and all other properties your custom widgets needs as well, like those custom TextStyles.

With theme extensions there is no reason why your custom widgets should not have its own theme if it is complex enough to warrant it. Going forward we may see that user interface packages come with their own theme extension.

Theme extensions completely changes what you can do with theming in Flutter, you can finally build that design system you always wanted to make to speed up your development, and ensure easy to reuse design consistency.
...
In the chapter [Roads to ThemeData](/theming_roads) where we look at all the different ways to define a `ThemeData` object. We will see different result like these, depending on how we create our **ThemeData** the results can vary a lot, even if we use same input colors.
...
Don't do this, it was something that was used way back, before there was a `darkTheme` property in the `MaterialApp`. Prefer to assign your theme mode appropriate themes to the correct properties in the `MaterialApp`. This also applies to the `highContrastTheme` and `highContrastDarkTheme` theme. If your app has special designs for high contrast accessibility themes, do assign them to these properties. These properties are all there to give you the right device system settings driven behavior. So your app can follow device system theme mode and its high contrast accessibility setting. Currently, only iOS supports system setting based switching to theme provided via the `highContrastTheme` and `highContrastDarkTheme` theme properties. 
