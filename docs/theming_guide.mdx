...
The chapter [ThemeData and ColorScheme](/themedata_colorscheme) goes through different concepts and key aspect of **ThemeData** and **ColorScheme** and provides inisights in which constructors to use to make correct M2 and M3 theme with them. In the [Roads to ThemeData](/theming_roads) chapter, we look deeper at different ways to make a `ThemeData` object and analyze what their differences are, and demonstrate why the suggested methods produce the best results. 
...
A basic use case is adding more colors to the theme that your application needs, it is a very handy way to add semantic colors to your theme. There is no need to limit yourself to that though. You can add any other property values your custom widgets need and custom text styles too.
...
Theme extensions completely change what you can do with theming in Flutter. You can finally build a design system that uses the Flutter theming engine. This may speed up your development and ensure reusable design consistency across development teams.
