...
Starting from version 5.1.0 FlexColorScheme support the Theme Extensions API directly as well. It is just a convenience pass through to the `ThemeData` API so you don't have to add it with an extra `copyWith`. You use it exactly the same way as the extensions in ThemeData. The [default example](/examples#theme-extensions) includes a small example on how to use it.
